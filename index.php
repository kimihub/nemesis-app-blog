...
/*
	nemesis-app-example
	Description : lightweight blog built on jQuery
	Dependencies :
		nemesis-framework/core
			...
			core/class.MVC.php
			core/class.App.php
			core/class.CSSmin.php
			core/class.HTMLhelpers.php
			core/functions.php
*/
...
class blog extends App
{
  ...
  private $session = null;
    ...
    chmod($this->path.'uploads/', 0777);
    chmod($this->path.'uploads/thumbnails/', 0777);
		// perms
    ...
    */
		...
		if (!URL::isHttpRequest())
		{
			$this->injectVar('head', array(
				...
				'jquery' => JQUERY,
				'favicon' => '',
				'font' => NEMESIS_URL.$this->url.FONT_NAME,
			));
	...
	public function login()
	{
        ...
        */
		...
		else
		{
			...
			{
				$this->addMessage('info', 'Already logged in!');
				$this->displayMessages();
				exit();
			}
		}
	...
	public function post_add($id="")
	{
		if ($this->isLogged())
		{
			...
			$this->injectCol('post_add', 'HTML', new HTMLhelpers());
	...
	public function dropbox($arguments=array())
	{
		if ($this->isLogged())
		{
			...
			$this->injectCol('dropbox', 'HTML', new HTMLhelpers());
			$this->addToBuffer($this->getView('html'));
